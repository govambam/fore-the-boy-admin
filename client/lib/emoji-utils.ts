...
export function getSafeEmoji(emojiDef: EmojiWithFallback): string {
  ...
  const safeOptions = SAFE_FALLBACKS[emojiDef.description.replace(' ', '_').toUpperCase() as keyof typeof SAFE_FALLBACKS];
 ...
 */
export function getContestTypeWithEmoji(contestType: 'longDrive' | 'closestToPin'): string {
  const config = contestType === 'longDrive' ? GOLF_EMOJIS.LONG_DRIVE : GOLF_EMOJIS.CLOSEST_TO_PIN;
 ...
 */
export function getContestEmoji(contestType: 'longDrive' | 'closestToPin'): string {
  const config = contestType === 'longDrive' ? GOLF_EMOJIS.LONG_DRIVE : GOLF_EMOJIS.CLOSEST_TO_PIN;
...
export function preloadEmojiSupport(): void {
  ...
  setTimeout(() => {
    ...
    Object.values(SAFE_FALLBACKS).flat().forEach(emoji => {
